...
for idx = 1:length(alpha_percobaan)
    ...
    for k = 1:iterasi
        ...
        % Perhitungan pi'(xk)
        gradient_pi_aksen = double(subs(gradient_sym, x, x_curr)); % Gradient di titik x_curr

        % Rumus update rule: x(k+1)= xk + α · π'(xk)
        x_curr = x_curr + a * gradient_pi_aksen; % Update rule
    end
